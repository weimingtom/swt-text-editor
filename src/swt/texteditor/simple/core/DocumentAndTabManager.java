...
public class DocumentAndTabManager {
    ...
    public Map<Document, CTabItem> getDocumentsToTabs() {
        synchronized (documentsToTabs) {
            return documentsToTabs;
        }
    }

    private final Map<Document, CTabItem> documentsToTabs;

    public Document getDocument(CTabItem cTabItem) {
        synchronized (documentsToTabs) {
            for (Map.Entry<Document, CTabItem> docToTabEntry : documentsToTabs.entrySet()) {
    ...
    public CTabItem addOrOpenExisting(Document document) {
        ...
        if (LOGGER.isInfoEnabled()) LOGGER.info(tab + " <-> " + document.getPath());
        return tab;
    }
    ...
    public CTabItem saveAs(final CTabItem tab, Document document, String newPath) {
        ...
        if (LOGGER.isDebugEnabled()) LOGGER.debug("Saving document as " + newDocument.getPath());
        try {
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("Saving file " + newDocument.getPath());
            FileUtils.write(newDocument.getPath(), newDocument.getContent());
            if (LOGGER.isDebugEnabled()) LOGGER.debug("Saved file " + newDocument.getPath());
            synchronized (documentsToTabs) {
    ...
    public CTabItem save(CTabItem tab, Document document) {
        if (LOGGER.isDebugEnabled()) LOGGER.debug("Saving document " + document.getPath());
        return saveAs(tab, document, document.getPath().getAbsolutePath());
    }

    public void reload(Document document) {
        if (LOGGER.isDebugEnabled()) LOGGER.debug("Reloading " + document.getPath());
        try {
            final String content = FileUtils.readFileToString(document.getPath());
            ...
            synchronized (documentsToTabs) {
                ...
                documentsToTabs.put(newDocument, cTabItem);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (LOGGER.isDebugEnabled()) LOGGER.debug("Hopefully reloaded " + document.getPath());
    }
