...
public class DocumentTabDisposeListener implements DisposeListener {
    ...
    public DocumentTabDisposeListener(DocumentAndTabManager documentAndTabManager) {
        this.documentAndTabManager = documentAndTabManager;
    }

    @Override
    public void widgetDisposed(DisposeEvent disposeEvent) {
        if (LOGGER.isDebugEnabled()) LOGGER.debug("Disposing tab" + disposeEvent);
        final CTabItem tab = (CTabItem) disposeEvent.getSource();
        documentAndTabManager.removeDocumentByTab(tab);
        if (LOGGER.isDebugEnabled()) LOGGER.debug("All related tabs should be removed." + disposeEvent);
    }
}
